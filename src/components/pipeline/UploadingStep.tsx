...
export function UploadingStep({ status }: UploadingStepProps) {
  ...
  return (
    <StepCard $status={status}>
      <IconWrapper $status={status}>{isCompleted ? <CheckCircle size={24} /> : <Upload size={24} />}</IconWrapper>

      <StepContent>
        <StepTitle $status={status}>Uploading</StepTitle>
        <StepDescription $status={status}>コードをS3バケットにアップロードしています</StepDescription>
  ...
  border: 2px solid
    ${(props) =>
      props.$status === 'completed'
        ...
        : props.$status === 'active'
          ? props.theme.color.statusUploading
